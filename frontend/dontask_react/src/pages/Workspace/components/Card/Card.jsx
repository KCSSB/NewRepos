...
// Все доступные приоритеты
const ALL_PRIORITIES = ["Низкий", "Средний", "Высокий"];
...
// 🔑 НОВЫЙ КОМПОНЕНТ: Выбор приоритета в режиме редактирования
const PrioritySelect = ({ taskId, currentPriority, updatePriorityChange }) => {
  // Обработчик выбора приоритета
  const handleSelectPriority = useCallback(
    (priority) => {
      updatePriorityChange(taskId, priority);
    },
    [taskId, updatePriorityChange]
  );

  return (
    <div className="priority-select-container">
      <div className="priority-select-label">Приоритет:</div>
      <div className="priority-options-group">
        {ALL_PRIORITIES.map((priority) => (
          <button
            ...
            className={`priority-option-button priority-${priority.toLowerCase()} ${
...
const TaskItem = ({ card }) => {
  ...
  const { isEditMode, cardNameChanges } = useWorkspaceEdit();
...
const ListColumn = ({ list, index }) => {
  const { createTask, loading, deleteCardColumn } = useWorkspace();
  ...
  // 🔑 НОВЫЙ ОБРАБОТЧИК: Удаление колонки
  const handleDeleteColumn = async () => {
    if (
      window.confirm(
        `Вы уверены, что хотите удалить колонку "${list.listName}"?`
      )
    ) {
      await deleteCardColumn(list.listId);
    }
  };
    ...
    <div className="existing-list-container">
      ...
      <div className="list-header-wrapper">
        {isEditMode ? (
          ...
          <h3 className="list-title-and-button">
            {list.listName || `ID: ${list.listId}`}
            <button
              type="button"
              className="delete-list-button" // Новый класс для стилизации
              onClick={handleDeleteColumn}
              title={`Удалить колонку "${list.listName}"`}
            >
              <img
                src={delete_icon}
                alt="Удалить колонку"
                className="delete-list-icon-img"
              />
            </button>
          </h3>
        )}
